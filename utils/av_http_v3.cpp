	...
	Result Client::request(Method method, const std::string &url, const Header *header, const Form *form,
		...
		if (header != nullptr)
			for (const auto &h: header->kv) {
				...
				const auto key_lower = av::str::toLower(h.first);
		...
		av::async::Exit exit_mime_part([&part] {
			// just ref
		});
		...
		if (m_timeout.has_value()) {
			curl_easy_setopt(curl, CURLOPT_TIMEOUT_MS, m_timeout.value()); // Timeout in seconds
		}
